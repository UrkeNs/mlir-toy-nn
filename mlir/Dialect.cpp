...
// PredictOp
//===----------------------------------------------------------------------===//
static mlir::toy::CreateModelOp findCreateModel(mlir::Value model) {
  while (model) {
    if (auto cm = model.getDefiningOp<mlir::toy::CreateModelOp>())
      return cm;
...
void PredictOp::build(mlir::OpBuilder &builder,
  mlir::OperationState &state,
  mlir::Value model,
  mlir::Value x) {
// Operands: (model, x)
state.addOperands({model, x});
// Result type: start unranked, shape inference can refine
state.addTypes(mlir::UnrankedTensorType::get(builder.getF64Type()));
}


void mlir::toy::PredictOp::inferShapes() {
  auto xTy = llvm::dyn_cast<mlir::RankedTensorType>(getX().getType());
  if (!xTy || xTy.getRank() != 2)
    return;

  int64_t batch = xTy.getDimSize(0);

  auto cm = findCreateModel(getModel());
  if (!cm)
    return;

  int64_t outDim = cm.getOutputAttr().getInt();
  auto outTy = mlir::RankedTensorType::get({batch, outDim}, xTy.getElementType());
  getResult().setType(outTy);
}


mlir::LogicalResult PredictOp::verify() {
  // Operand0 must be !toy.model
  if (!llvm::isa<ModelType>(getModel().getType()))
    return emitOpError("first operand must be !toy.model");

  // If x is ranked, enforce rank-2 [batch, features] for now
  auto xType = llvm::dyn_cast<mlir::RankedTensorType>(getX().getType());
  if (!xType)
    return mlir::success();

  if (xType.getRank() != 2)
    return emitOpError("expects rank-2 input tensor [batch, features]");
...
mlir::LogicalResult CreateModelOp::verify() {
  if (getInput() <= 0 || getDepth() <= 0 || getWidth() <= 0 || getOutput() <= 0)
    return emitOpError("input/depth/width/output must all be > 0");
  return mlir::success();
}

void CreateModelOp::build(mlir::OpBuilder &builder,
  mlir::OperationState &state,
  int64_t input, int64_t depth,
  int64_t width, int64_t output) {
state.addTypes(mlir::toy::ModelType::get(builder.getContext()));
state.addAttribute("input",  builder.getI64IntegerAttr(input));
state.addAttribute("depth",  builder.getI64IntegerAttr(depth));
state.addAttribute("width",  builder.getI64IntegerAttr(width));
state.addAttribute("output", builder.getI64IntegerAttr(output));
}
...
mlir::LogicalResult MakeStructOp::verify() {
auto structTy = llvm::dyn_cast<mlir::toy::StructType>(getResult().getType());
if (!structTy)
return emitOpError("result must be a !toy.struct<...>");

if (structTy.getNumElementTypes() != getInputs().size())
return emitOpError("number of operands must match struct element count");

for (auto it : llvm::enumerate(getInputs())) {
if (it.value().getType() != structTy.getElementTypes()[it.index()])
return emitOpError("operand type mismatch at index ")
<< it.index() << ": got " << it.value().getType()
<< " expected " << structTy.getElementTypes()[it.index()];
}
return success();
}
...
mlir::Type ToyDialect::parseType(mlir::DialectAsmParser &parser) const {
  ...
  if (succeeded(parser.parseOptionalKeyword("model"))){
...
void ToyDialect::printType(mlir::Type type,
  ...
  if (llvm::isa<mlir::toy::ModelType>(type)) {
    printer << "model";
    return;
  }
