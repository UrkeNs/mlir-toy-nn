...
private:
  ...
  //struktura koja opisuje model
  struct SequentialConfig{
    int64_t input;//broj inputa
    int64_t depth;//broj layera ukljucujuci i output layer
    int64_t width;//broj neurona u jednom skrivenom layeru
    int64_t output;// broj outputa
  };

  // sequentialModels nema nikakv runtime znacaj. Tokom kompajliranja programa govori specifikacije modela.
  llvm::StringMap<SequentialConfig> sequentialModels;
  
  
  std::optional<SequentialConfig> parseSequentialConfig(CallExprAST &call){
    //Ako model = Sequential(args..) nema 4 argumenta odma se odbacuje 
    if(call.getArgs().size() != 4){
      return std::nullopt; //vraca se prazan cfg
    }
    //Toy jezik podrzava samo double tip podatka tako da je potrebno da se uradi converzija u int tako sto ce se samo odbaciti deo iza zareze
    auto getInt = [](ExprAST *e) -> std::optional<int64_t>{
      if(auto *num = llvm::dyn_cast<NumberExprAST>(e)) {
        ...
        if (static_cast<double>(iv) != v) return std::nullopt; // Ovaj deo je veoma bitan, ovde se vrsi provera da li je double broj stvarno int(npr. 1.0, 4.0 itd) 
        return iv;
      }
      return std::nullopt;
    };

    //U ovom delu se vade svi argumenti iz Sequential(input, depth, width, output)
    auto a0 = getInt(call.getArgs()[0].get());
    auto a1 = getInt(call.getArgs()[1].get());
    auto a2 = getInt(call.getArgs()[2].get());
    auto a3 = getInt(call.getArgs()[3].get());
    if (!a0 || !a1 || !a2 || !a3)
      return std::nullopt;

    //pravi sequentialConfig i vraca
    SequentialConfig cfg{*a0, *a1, *a2, *a3};
    if (cfg.input <= 0 || cfg.depth <= 0 || cfg.width <= 0 || cfg.output <= 0)
      return std::nullopt;
  ...
  mlir::toy::FuncOp mlirGen(PrototypeAST &proto) {
    ...
    // --- Force main(arg0) dataset to be rank-2 tensor<?x?xf64> ---
    if (proto.getName() == "main" && !argTypes.empty()) {
      if (auto unranked = llvm::dyn_cast<mlir::UnrankedTensorType>(argTypes[0])) {
        // Keep element type (should be f64 in your case)
        argTypes[0] = mlir::RankedTensorType::get(
    ...
    return mlir::toy::FuncOp::create(builder, location, proto.getName(), funcType);
  }
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    if (callee == "predict") {
      if (call.getArgs().size() != 2) {
        emitError(location,
                  "MLIR codegen encountered an error: toy.predict expects 2 "
                  "arguments: predict(model, x)");
        return nullptr;
      }
    
      mlir::Value modelVal = mlirGen(*call.getArgs()[0]);
      if (!modelVal)
        return nullptr;
    
      mlir::Value xVal = mlirGen(*call.getArgs()[1]);
      if (!xVal)
        return nullptr;
      ...
      if (!xType || xType.getRank() != 2) {
        emitError(location,
                  "MLIR codegen encountered an error: toy.predict expects x to be a "
                  "rank-2 tensor<batchxinxf64>");
        return nullptr;
      }
    ...
    if (callee == "create_model") {
      ...
      if (!cfgOpt) {
        emitError(location,
                  "create_model expects 4 integer args: "
                  "create_model(input, depth, width, output) with all values > 0");
        return nullptr;
      }
      auto cfg = *cfgOpt;
    
      return CreateModelOp::create(builder, location,
                                   cfg.input, cfg.depth, cfg.width, cfg.output);
    }
    ...
    if (callee == "train") {
      ...
      // Arg1: dataset tensor
      mlir::Value dataVal = mlirGen(*call.getArgs()[1]);
      if (!dataVal)
        return nullptr;
      ...
      auto epochsOpt = getInt(call.getArgs()[2].get());
      if (!epochsOpt || *epochsOpt <= 0) {
        emitError(location,
    ...
    // Codegen operands for normal calls.
    ...
    // Otherwise: user-defined function call
